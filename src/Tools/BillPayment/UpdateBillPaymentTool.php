<?php
/*
 * This file is part of the Laravel QuickBooks MCP package.
 *
 * Copyright (c) 2026 Raju Rayhan
 *
 * This source file is subject to the MIT license that is bundled
 * with this source code in the file LICENSE.
 */
namespace Raju\QuickBooksMcp\Tools\BillPayment;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;
use Raju\QuickBooksMcp\Services\QuickBooksService;

#[Description('Update an existing QuickBooks bill payment. Only pass fields you want to change.')]
class UpdateBillPaymentTool extends Tool
{
    public function schema(): array
    {
        return [
            'type'       => 'object',
            'properties' => [
                'id'           => ['type' => 'string', 'description' => 'QuickBooks bill payment ID (required)'],
                'txn_date'     => ['type' => 'string', 'description' => 'Payment date (YYYY-MM-DD)'],
                'total_amount' => ['type' => 'number', 'description' => 'Total payment amount'],
            ],
            'required' => ['id'],
        ];
    }

    public function handle(Request $request, QuickBooksService $qb): Response
    {
        try {
            $payment = $qb->findById('BillPayment', $request->get('id'));

            if (!$payment) {
                return Response::text("Bill payment ID {$request->get('id')} not found.");
            }

            if ($request->has('txn_date'))     $payment->TxnDate   = $request->get('txn_date');
            if ($request->has('total_amount')) $payment->TotalAmt  = $request->get('total_amount');

            $updated = $qb->update($payment);

            return Response::text(
                "Bill payment updated.\nID: {$updated->Id} | Amount: {$updated->TotalAmt} | Date: {$updated->TxnDate}"
            );

        } catch (\Exception $e) {
            return Response::text("Error: " . $e->getMessage());
        }
    }
}
