...
#[Description('Search QuickBooks bill payments by vendor name or date range.')]
class SearchBillPaymentsTool extends Tool
{
    ...
    public function handle(Request $request, QuickBooksService $qb): Response
    {
        try {
            ...
            $sql         = "SELECT * FROM BillPayment {$whereClause} MAXRESULTS {$limit}";

            $results = $qb->query($sql);

            if (empty($results)) {
                return Response::text("No bill payments found.");
            }

            $lines = collect($results)->map(fn($p) => implode(' | ', array_filter([
                "ID: {$p->Id}",
                $p->VendorRef->name ?? null,
                "Amount: {$p->TotalAmt}",
                "Date: {$p->TxnDate}",
                $p->PayType ?? null,
            ])))->join("\n");

            return Response::text("Found " . count($results) . " bill payment(s):\n\n{$lines}");
